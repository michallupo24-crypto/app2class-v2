...
const ParentRegistration = () => {
  ...
  const verifyChild = async () => {
    ...
    try {
      let data: { id: string; full_name: string }[] | null = null;

      if (childId) {
        const { data: rpcData } = await supabase.rpc("find_student_by_id_number", {
          p_id_number: childId,
          p_school_id: childSchoolId,
        });
        data = rpcData;
      } else {
  ...
  const parentIdValid = isValidIsraeliId(parentIdNumber);
  const step1Valid = firstName && lastName && phone && email && password.length >= 6 && parentIdValid && childName && childSchoolId && childVerified === true;
    ...
    >
      {step === 1 && (
        <div className="space-y-5">
          ...
          <div className="space-y-2">
            <Label className="font-heading">ת"ז (שלך)</Label>
            <Input
              value={parentIdNumber}
              onChange={(e) => setParentIdNumber(e.target.value.replace(/\D/g, "").slice(0, 9))}
              placeholder="9 ספרות" dir="ltr" required
            />
            {parentIdNumber.length === 9 && !parentIdValid && (
