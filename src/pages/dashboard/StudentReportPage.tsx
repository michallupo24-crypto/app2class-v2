...
const StudentReportPage = () => {
  ...
  const [educatorName, setEducatorName] = useState<string | null>(null);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // 3. Real attendance rate
      const { data: attRows } = await supabase
        .from("attendance")
        .select("status")
        .eq("student_id", studentId);
      const absences = (attRows || []).filter((a: any) => a.status === "absent").length;
      const attPct = attRows?.length ? Math.round(((attRows.length - absences) / attRows.length) * 100) : null;
      setAttendancePct(attPct);

      // 4. Real syllabus progress for the student's class (all subjects)
      let syllPct: number | null = null;
      if (profile?.class_id && profile?.school_id && profile?.classes?.grade) {
        const [{ data: topics }, { data: completed }] = await Promise.all([
          supabase.from("syllabi").select("id")
            .eq("school_id", profile.school_id).eq("grade", profile.classes.grade as any),
          supabase.from("class_syllabus_progress").select("syllabus_id")
            .eq("class_id", profile.class_id).eq("status", "completed"),
        ]);
        if (topics && topics.length > 0) {
          const completedIds = new Set((completed || []).map((c: any) => c.syllabus_id));
          syllPct = Math.round((topics.filter((t: any) => completedIds.has(t.id)).length / topics.length) * 100);
        }
      }
      setSyllabusPct(syllPct);

      // 5. Real homeroom educator name
      if (profile?.class_id) {
        const { data: educatorRole } = await supabase
          .from("user_roles")
          .select("profiles!inner(full_name)")
          .eq("role", "educator" as any)
          .eq("homeroom_class_id", profile.class_id)
          .limit(1)
          .maybeSingle();
        setEducatorName((educatorRole?.profiles as any)?.full_name || null);
      }
      ...
      const performanceLine = !submissions?.length
        ...
        : avg >= 90
          ? `${profile.full_name} מציג/ה ביצועים מצוינים ב${subjectsText || "מקצועות הלימוד"}, עם ממוצע של ${avg}.`
          : avg >= 70
            ? `${profile.full_name} שומר/ת על ממוצע תקין של ${avg} ב${subjectsText || "מקצועות הלימוד"}.`
            : `${profile.full_name} מציג/ה ממוצע של ${avg} ב${subjectsText || "מקצועות הלימוד"}, ויש מקום לשיפור.`;

      const attendanceLine = attPct === null
        ? ""
        : attPct >= 95 ? " הנוכחות מצוינת וסדירה." : attPct >= 85 ? " הנוכחות תקינה." : " כדאי לשים לב לשיפור הנוכחות.";

      setAiInsight(`${performanceLine}${attendanceLine}`);
  ...
  // fake key shared by every certificate.
  const verification = useMemo(() => {
    const src = studentId || "";
    let hash = 0;
    for (let i = 0; i < src.length; i++) hash = (hash * 31 + src.charCodeAt(i)) >>> 0;
    const cells = Array.from({ length: 16 }, (_, i) => ((hash >> i) & 1) === 1);
    const key = hash.toString(16).toUpperCase().padStart(8, "0");
    return { cells, key: `A2L8-${key.slice(0, 4)}-${key.slice(4, 8)}` };
  }, [studentId]);
