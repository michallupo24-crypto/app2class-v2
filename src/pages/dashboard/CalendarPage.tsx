...
const CalendarPage = () => {
  ...
  // Load school-wide events (trips, ceremonies, exam periods, school-specific holidays)
  useEffect(() => {
  ...
  // School-wide events for selected date (trips, ceremonies, exam periods, school holidays)
  ...
  const quizDays = gradeEvents
    ...
    .map(e => parseSchoolDate(e.event_date));
  const schoolEventRanges = schoolEvents.map(e => ({
    from: parseSchoolDate(e.start_date), to: parseSchoolDate(e.end_date),
  }));

  // Merge grade events + school-wide events into one upcoming feed
