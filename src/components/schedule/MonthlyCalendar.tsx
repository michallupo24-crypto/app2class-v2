...
type HolidayPeriod = {
  ...
  returnDate?: string; note?: string;
};

type GradeEvent = {
  id: string; event_date: string; title: string;
  event_type: string; subject: string | null; description: string | null;
};

type SchoolEvent = {
  id: string; title: string; description: string | null;
  start_date: string; end_date: string; is_holiday: boolean; event_type: string;
};
...
const EVENT_TYPE_CONFIG: Record<string, { color: string; icon: typeof BookOpen; label: string }> = {
  ...
  holiday: { color: "bg-info/10 border-info/20 text-info", icon: Sun, label: "חופשה" },
  exam_period: { color: "bg-destructive/10 border-destructive/20 text-destructive", icon: BookOpen, label: "תקופת בחינות" },
...
const gregorianFormatter = new Intl.DateTimeFormat("he-IL", { weekday: "long", day: "2-digit", month: "2-digit", year: "numeric" });
const hebrewFormatter = new Intl.DateTimeFormat("he-IL-u-ca-hebrew", { day: "numeric", month: "long", year: "numeric" });
const shortDateFormatter = new Intl.DateTimeFormat("he-IL", { weekday: "short", day: "2-digit", month: "2-digit" });

interface MonthlyCalendarProps {
  profile: UserProfile;
}

const MonthlyCalendar = ({ profile }: MonthlyCalendarProps) => {
  ...
  const { subjects: mySubjects, trackNames, hasMegamaA, hasMegamaB } = useStudentSubjects(
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (myProfile?.class_id) {
        const { data: cls } = await supabase.from("classes").select("grade").eq("id", myProfile.class_id).single();
        grade = cls?.grade || null;
      }
      if (!grade && !isStudent) {
        const { data: roles } = await supabase.from("user_roles").select("grade").eq("user_id", profile.id).not("grade", "is", null).limit(1);
        if (roles?.length) grade = roles[0].grade;
      }
      setStudentGrade(grade);
      if (!grade) return;
      const { data: events } = await supabase.from("grade_events")
        .select("id, event_date, title, event_type, subject, description")
        .eq("school_id", profile.schoolId).eq("grade", grade as any).eq("status", "approved")
        .order("event_date", { ascending: true });
      setGradeEvents(isStudent ? filterEventsBySubjects(events || [], mySubjects, hasMegamaA, hasMegamaB) : (events || []));
    };
    load();
  }, [profile.id, profile.schoolId, isStudent, mySubjects, hasMegamaA, hasMegamaB]);

  useEffect(() => {
    const loadSchoolEvents = async () => {
      let query = supabase
        .from("school_events")
        .select("id, title, description, start_date, end_date, is_holiday, event_type")
        .order("start_date", { ascending: true });
      query = profile.schoolId
        ? query.or(`school_id.eq.${profile.schoolId},school_id.is.null`)
  ...
  const holidayPeriods = useMemo(() =>
    HOLIDAY_PERIODS.map(p => ({
      ...p,
      startDate: parseSchoolDate(p.start),
      endDate: parseSchoolDate(p.end),
      returnDateObj: p.returnDate ? parseSchoolDate(p.returnDate) : null,
    })), []);

  const currentDate = startOfDay(new Date());

  const activeHoliday = holidayPeriods.find(h =>
    currentDate.getTime() >= startOfDay(h.startDate).getTime() && currentDate.getTime() <= startOfDay(h.endDate).getTime()
  );
  ...
  const selectedDaySchoolEvents = useMemo(() => {
    if (!selectedDate) return [];
    const day = startOfDay(selectedDate).getTime();
    return schoolEvents.filter(e => {
      const start = startOfDay(parseSchoolDate(e.start_date)).getTime();
      const end = startOfDay(parseSchoolDate(e.end_date)).getTime();
  ...
  const selectedDayHolidays = useMemo(() => {
    ...
    return holidayPeriods.flatMap(period => {
      ...
      if (selectedDay.getTime() >= start.getTime() && selectedDay.getTime() <= end.getTime()) {
        events.push({ id: `${period.id}-holiday`, title: period.title, type: "holiday", subtitle: `${gregorianFormatter.format(period.startDate)} - ${gregorianFormatter.format(period.endDate)}` });
      }
      if (period.returnDateObj && isSameDay(selectedDay, period.returnDateObj)) {
        events.push({ id: `${period.id}-return`, title: `חזרה ללימודים אחרי ${period.title}`, type: "return", subtitle: gregorianFormatter.format(period.returnDateObj) });
      }
      return events;
    });
  ...
  const examDays = gradeEvents.filter(e => ["exam", "bagrut"].includes(e.event_type)).map(e => parseSchoolDate(e.event_date));
  const quizDays = gradeEvents.filter(e => e.event_type === "quiz").map(e => parseSchoolDate(e.event_date));
  const schoolEventRanges = schoolEvents.map(e => ({ from: parseSchoolDate(e.start_date), to: parseSchoolDate(e.end_date) }));

  const allUpcomingEvents = useMemo(() => {
    const fromGrade = gradeEvents.map(e => ({ id: e.id, title: e.title, event_type: e.event_type, subject: e.subject, event_date: e.event_date }));
    const fromSchool = schoolEvents.map(e => ({ id: e.id, title: e.title, event_type: e.event_type, subject: null as string | null, event_date: e.start_date }));
    return [...fromGrade, ...fromSchool].sort((a, b) => parseSchoolDate(a.event_date).getTime() - parseSchoolDate(b.event_date).getTime());
  }, [gradeEvents, schoolEvents]);

  const filteredUpcoming = useMemo(() => {
    const future = allUpcomingEvents.filter(e => parseSchoolDate(e.event_date).getTime() >= currentDate.getTime());
    if (activeTab === "all") return future;
    if (activeTab === "exams") return future.filter(e => ["exam", "bagrut", "quiz"].includes(e.event_type));
    if (activeTab === "events") return future.filter(e => ["event", "trip", "ceremony", "holiday", "exam_period"].includes(e.event_type));
    if (activeTab === "deadlines") return future.filter(e => e.event_type === "deadline");
    return future;
  }, [allUpcomingEvents, currentDate, activeTab]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Card>
          <CardContent className="p-4 flex justify-center">
            <Calendar
              ...
              modifiers={{ holiday: holidayRanges, returnDay: returnDays, examDay: examDays, quizDay: quizDays, schoolEvent: schoolEventRanges }}
              modifiersClassNames={{
                ...
                schoolEvent: "bg-accent/40 text-accent-foreground font-semibold",
              }}
            />
          </CardContent>
        </Card>
        ...
        <div className="space-y-4">
          <Card>
            ...
            <CardContent>
              {selectedDayHolidays.length === 0 && selectedDayGradeEvents.length === 0 && selectedDaySchoolEvents.length === 0 ? (
                <p className="text-sm text-muted-foreground py-4 text-center">אין אירועים ביום זה</p>
              ) : (
                <div className="space-y-2">
                  {selectedDaySchoolEvents.map(event => {
                    ...
                    return (
                      <div key={event.id} className={`p-3 rounded-lg border ${cfg.color}`}>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          {event.description && <span className="text-[10px] opacity-80">{event.description}</span>}
                        </div>
                      </div>
                    );
                  })}
                  {selectedDayHolidays.map(event => (
                    <div key={event.id} className={`p-3 rounded-lg border ${event.type === "holiday" ? "bg-info/10 border-info/20 text-info" : "bg-primary/10 border-primary/20 text-primary"}`}>
                      <div className="flex items-center gap-2">
                  ...
                  {selectedDayGradeEvents.map(event => {
                    ...
                    return (
                      <div key={event.id} className={`p-3 rounded-lg border ${cfg.color}`}>
                        <div className="flex items-center gap-2">
                          <Icon className="h-4 w-4 shrink-0" />
                          <span className="font-heading font-medium text-sm">{isStudent ? personalizeEventTitle(event.title, event.subject, trackNames) : event.title}</span>
                        </div>
                        <div className="flex items-center gap-2 mt-1">
                          <Badge variant="outline" className="text-[9px] px-1.5 h-4">{cfg.label}</Badge>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-body truncate">{isStudent ? personalizeEventTitle(event.title, event.subject, trackNames) : event.title}</p>
