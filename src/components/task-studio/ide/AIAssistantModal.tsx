...
export const AIAssistantModal: React.FC<AIAssistantModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-[#111111] border border-[#2A2A2A] rounded-lg w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh] text-[#E0E0E0]">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-semibold transition cursor-pointer ${
                tab === 'prompt' ? 'bg-accent text-accent-foreground' : 'text-gray-400 hover:text-white hover:bg-[#2A2A2A]'
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-semibold transition cursor-pointer ${
                tab === 'debug' ? 'bg-destructive text-white' : 'text-gray-400 hover:text-white hover:bg-[#2A2A2A]'
              }`}
            >
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1">
          {errorMessage && (
            <div className="bg-destructive/10 border border-destructive/30 text-destructive p-3 rounded-lg text-xs">
              {errorMessage}
            </div>
          )}
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  ...
                  className="w-full bg-[#0A0A0A] border border-[#2A2A2A] rounded-lg p-2.5 text-xs font-medium text-white focus:outline-none"
                >
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-[#0A0A0A] border border-[#2A2A2A] rounded-lg p-3 text-xs text-white focus:outline-none focus:border-accent"
                />
              </div>
              ...
              <button
                ...
                className="w-full bg-accent hover:opacity-90 disabled:opacity-50 text-accent-foreground font-semibold py-2.5 rounded-lg transition cursor-pointer flex items-center justify-center gap-2 text-xs"
              >
                {isLoading ? (
                  <>
              ...
              {generatedResult && (
                <div className="bg-[#0A0A0A] border border-[#2A2A2A] rounded-lg p-4 text-gray-300 space-y-3">
                  <div className="flex items-center justify-between border-b border-[#2A2A2A] pb-2">
                    <h4 className="font-bold text-accent text-xs">{generatedResult.title || 'תוצר אינטראקטיבי חדש'}</h4>
                    <span className="text-[10px] bg-accent/20 text-accent px-2 py-0.5 rounded border border-accent/30">מוכן לייבוא</span>
                  </div>
                  <p className="text-xs text-gray-400">{generatedResult.description}</p>

                  <button
                    onClick={handleApplyGen}
                    className="w-full bg-primary text-primary-foreground hover:opacity-90 font-semibold py-2 rounded-lg transition cursor-pointer flex items-center justify-center gap-2 text-xs"
                  >
          ...
          {tab === 'debug' && (
            <div className="space-y-4">
              <div className="bg-[#1A1A1A] border border-[#2A2A2A] p-3 rounded-lg text-gray-400 text-xs leading-relaxed">
              ...
              <div className="bg-[#0A0A0A] text-gray-300 p-3 rounded-lg text-xs font-mono max-h-32 overflow-y-auto border border-[#2A2A2A]">
                ...
                {consoleLogs.length === 0 ? (
                  ...
                  consoleLogs.map((l) => (
                    <div key={l.id} className={l.level === 'error' ? 'text-destructive font-semibold' : 'text-gray-400'}>
              ...
              <button
                ...
                disabled={isLoading}
                className="w-full bg-destructive hover:opacity-90 text-white font-semibold py-2.5 rounded-lg transition cursor-pointer flex items-center justify-center gap-2 text-xs"
              >
                {isLoading ? (
                  <>
                    <Loader2 className="w-4 h-4 animate-spin" />
              ...
              {debugResult && (
                <div className="bg-[#0A0A0A] border border-[#2A2A2A] rounded-lg p-4 text-gray-300 space-y-3">
                  ...
                  <div className="text-xs text-success bg-success/10 p-3 rounded-lg border border-success/20">
