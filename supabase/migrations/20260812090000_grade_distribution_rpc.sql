-- Spec: "Anonymous Bell Curve" (parent grade-benchmark dashboard) — an abstract
-- graph showing how many students scored in each grade range, with the child's
-- own average positioned on it. Explicitly requires full anonymization: never
-- leak other students' identities, only return aggregate counts per bucket.
-- Mirrors get_class_average / get_assignment_averages' SECURITY DEFINER pattern.
...
begin
  ...
  select bucket_label, bucket_min, count(*)::int as student_count
  from bucketed
...
revoke execute on function public.get_grade_distribution(uuid, text) from public, anon;
grant execute on function public.get_grade_distribution(uuid, text) to authenticated;
